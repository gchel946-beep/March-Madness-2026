...
import {
  getUpsetAnalysis, getMatchupAdvantages,
  offColor, defColor, toColor, t3Color, kpColor, winProb,
} from "@/lib/analytics";

/* ── StatCell ────────────────────────────────────────────── */
function StatCell({ label, value, rank, colorClass }: {
  label: string; value: string|number; rank: number; colorClass: string;
}) {
  return (
...
function SLabel({ text }: { text: string }) {
  return (
    <div className="font-mono text-[9px] tracking-widest text-tx3 uppercase pb-1 border-b border-br mt-3 mb-2">
      {text}
    </div>
  );
}

/* ── Dynamic analysis for later-round matchups ───────────── */
function generateAnalysis(t1: string, t2: string, favName: string, dogName: string, favWinPct: number): string {
  const d1 = getTeam(t1);
  const d2 = getTeam(t2);
  const parts: string[] = [];

  parts.push(`${favName} is the favorite at ${favWinPct}% to win based on seed and efficiency metrics.`);
  ...
  if (Math.abs(d1.aD - d2.aD) > 3) {
    ...
    parts.push(`${betterD} holds the defensive edge in this matchup.`);
  }
  if (Math.abs(d1.to - d2.to) > 2) {
    const lowTO  = d1.to < d2.to ? t1 : t2;
    const highTO = d1.to < d2.to ? t2 : t1;
    parts.push(`Ball security is a factor: ${lowTO} protects the ball better while ${highTO}'s turnover rate is a vulnerability.`);
  }
  if (Math.abs(d1.tp - d2.tp) > 5) {
    const faster = d1.tp > d2.tp ? t1 : t2;
    const slower = d1.tp > d2.tp ? t2 : t1;
    parts.push(`Pace mismatch: ${faster} wants to push the tempo while ${slower} prefers a halfcourt game.`);
  }
  return parts.join(" ");
}
    ...
    <div className="w-[380px] min-w-[380px] border-l border-br bg-s1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-6 text-tx3">
        ...
        <div className="mt-5 flex flex-col gap-2 text-left w-full">
          {[
            { label:"Blue bar", desc:"title contender",  cls:"text-blue border-blue/30 bg-blue/10" },
            { label:"Gold bar", desc:"dark horse",        cls:"text-gld border-gld/30 bg-gld/10" },
            { label:"Red bar",  desc:"upset candidate",   cls:"text-red border-red/30 bg-red/10" },
          ].map(i => (
...
export default function Sidebar() {
  ...
  if (!t1 && !t2) return <NeedsEarlierPicks />;
  if (!t1) { t1 = t2; t2 = ""; }
  ...
  // Upset analysis — seed-based favorite/underdog identification
  const ua = d2 ? getUpsetAnalysis(t1, t2, md?.uc) : null;
  const adv = d2 ? getMatchupAdvantages(t1, t2) : null;

  // Analysis text
  const analysisText = md?.ex
    ? md.ex
    : (d2 && ua ? generateAnalysis(t1, t2, ua.favorite, ua.underdog, ua.favWinPct) : "");
  ...
  const riskS = ua ? riskStyles[ua.level] : riskStyles.L;
    ...
    <div className="w-[380px] min-w-[380px] border-l border-br bg-s1 flex flex-col overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-br flex-shrink-0">
        ...
        <div className="text-[10px] text-tx3 mt-0.5">
          {roundLabel}{d1.r ? ` · ${d1.r}` : ""}{d2?.r ? ` · ${d2.r}` : ""}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        ...
        <div className="flex items-start gap-2 mb-3">
          <div className={[
            "flex-1 bg-s2 border rounded p-2 text-center",
            ua && ua.favorite === t1 ? "border-blue/40" : "border-br",
          ].join(" ")}>
            <div className="font-mono text-[9px] text-tx3 mb-0.5">#{d1.s} SEED</div>
            {ua && (
              <div className={`font-mono text-[8px] mb-1 ${ua.favorite===t1?"text-blue":"text-gld"}`}>
                {ua.favorite===t1 ? "▲ FAVORITE" : "▼ UNDERDOG"}
              </div>
            )}
            <div className="text-base font-bold leading-tight text-white">{t1}</div>
            <div className="font-mono text-[9px] text-tx3 mt-1">{d1.r}</div>
            {ua && d2 && (
              <div className={`font-mono text-[10px] font-semibold mt-1 ${ua.favorite===t1?"text-blue":"text-red"}`}>
                {ua.favorite===t1 ? ua.favWinPct : ua.upsetPct}%
              </div>
            )}
          </div>
          ...
          {d2 && ua && (
            <>
              <div className="text-[11px] font-semibold text-tx3 pt-5 flex-shrink-0">vs</div>
              <div className={[
                "flex-1 bg-s2 border rounded p-2 text-center",
                ua.favorite === t2 ? "border-blue/40" : "border-br",
              ].join(" ")}>
                <div className="font-mono text-[9px] text-tx3 mb-0.5">#{d2.s} SEED</div>
                <div className={`font-mono text-[8px] mb-1 ${ua.favorite===t2?"text-blue":"text-gld"}`}>
                  {ua.favorite===t2 ? "▲ FAVORITE" : "▼ UNDERDOG"}
                </div>
                <div className="text-base font-bold leading-tight text-white">{t2}</div>
                <div className="font-mono text-[9px] text-tx3 mt-1">{d2.r}</div>
                <div className={`font-mono text-[10px] font-semibold mt-1 ${ua.favorite===t2?"text-blue":"text-red"}`}>
                  {ua.favorite===t2 ? ua.favWinPct : ua.upsetPct}%
                </div>
              </div>
            </>
          )}
        ...
        {ua && d2 && (
          <>
            <SLabel text={`Upset Watch — ${ua.underdog} (#${ua.dogSeed} seed)`} />
            <div className={`border rounded p-3 mb-3 ${riskS.wrap}`}>

              {/* Tier badge */}
              <div className={`font-mono text-[9px] tracking-widest uppercase mb-2 ${riskS.badge}`}>
                {ua.label} UPSET RISK
                {ua.is89 && <span className="ml-2 opacity-70">· Near-Even Game</span>}
                {!ua.modelAgrees && <span className="ml-2 opacity-70">· Model Favors Underdog</span>}
              </div>

              {/* Favorite vs underdog rows */}
              <div className="flex items-center gap-3 mb-2">
                <div className="flex-1">
                  <div className="font-mono text-[8px] text-tx3 uppercase mb-0.5">Favorite</div>
                  <div className="text-[12px] font-semibold text-tx">#{ua.favSeed} {ua.favorite}</div>
                </div>
                <div className="text-center">
                  <div className="font-mono text-[8px] text-tx3 uppercase">Win %</div>
                  <div className="text-[11px] font-bold text-blue">{ua.favWinPct}%</div>
                </div>
              </div>

              <div className="flex items-center gap-3 mb-3">
                <div className="flex-1">
                  <div className="font-mono text-[8px] text-tx3 uppercase mb-0.5">Underdog</div>
                  <div className="text-[12px] font-semibold text-tx">#{ua.dogSeed} {ua.underdog}</div>
                </div>
                <div className="text-center">
                  <div className={`font-mono text-[8px] uppercase ${riskS.badge}`}>Upset %</div>
                  <div className={`text-[22px] font-bold leading-none ${riskS.badge}`}>{ua.upsetPct}%</div>
                </div>
              </div>

              {/* Bar */}
              <div className="h-1.5 bg-s3 rounded overflow-hidden mb-2">
                <div className={`h-full rounded transition-all ${riskS.bar}`} style={{width:`${ua.upsetPct}%`}} />
              </div>

              {/* Context note (model vs seed disagreement) */}
              {ua.contextNote && (
                <div className="text-[10px] text-tx3 leading-relaxed mt-2 border-t border-br/50 pt-2 italic">
                  {ua.contextNote}
                </div>
              )}
              ...
              {/* Analysis text */}
              {analysisText && (
                <div className="text-[11px] text-tx2 leading-relaxed mt-2 border-t border-br/50 pt-2">
                  {analysisText}
                </div>
              )}
            </div>
        ...
        {adv && d2 && (
          <>
            <SLabel text="Matchup Edge" />
            <div className="bg-s2 border border-br rounded p-2.5 mb-1">
              <div className="mb-2 text-[10px] font-semibold text-tx">
